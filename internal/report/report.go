...
func WriteText(w io.Writer, result model.Report) error {
	...
	if result.Snapshot.Virtualization.QEMUDetected || result.Snapshot.Virtualization.KVMAvailable || len(result.Snapshot.Virtualization.VirtualMachines) > 0 {
		...
		for _, vm := range result.Snapshot.Virtualization.VirtualMachines {
			fmt.Fprintf(w, "VM: %s %s running=%t vCPU %d CPU %.1f%% cgroup %.1f%% memory %.1f GiB current %.1f GiB RSS %.1f MiB I/O %.1f/%.1f MiB balloon %.1f/%.1f GiB QMP %s\n", vm.Name, vm.Source, vm.Running, vm.ConfiguredVCPUs, vm.CPUPercent, vm.CgroupCPUPercent, float64(vm.ConfiguredMemoryBytes)/(1024*1024*1024), float64(vm.MemoryCurrentBytes)/(1024*1024*1024), float64(vm.ProcessRSSBytes)/(1024*1024), float64(vm.ReadBytes)/(1024*1024), float64(vm.WriteBytes)/(1024*1024), float64(vm.BalloonActualBytes)/(1024*1024*1024), float64(vm.BalloonTargetBytes)/(1024*1024*1024), vm.QMPStatus)
